package com.mayhem.rs2.content.io;
...
import com.mayhem.rs2.content.quest.QuestManager.Quests;
...
import com.mayhem.rs2.entity.Entity.AttackType;
	...
	public static final class PlayerContainer {
		...
		public static boolean loadDetails(Player player) throws Exception {
			...
			try {
				...
				{
					player.getLootBag().setItems(details.lootingBag);
				}
		...
		private final List<Item> lootingBag;
		...
		public PlayerContainer(Player player) {
			...
			lootingBag = player.getLootBag().getItems();
	...
	public static final class PlayerDetails {
		...
		public static boolean loadDetails(Player player) throws Exception {
			...
			try {
				...
				player.ipSet = details.ipSet;
				player.nameSet = details.nameSet;
				player.recoverySet = details.recoverySet;
				player.emailSet = details.emailSet;
				player.setCredits(details.credits);
				player.setMoneySpent(details.moneySpent);
				
				//Set actual points first
				...
				player.setpuroPoints(details.puroPoints);
				...
				player.setSufferingCharges(details.sufferingCharges);
				...
				if(details.eloRating != 0) {
					player.eloRating = details.eloRating;
				}
				
				else
					player.eloRating = EloRatingSystem.DEFAULT_ELO_START_RATING;
				player.setRecallLocation(details.recallLocation);
				...
				player.setExtreme(details.isExtreme);
				player.setYearCreated(details.yearCreated);
				player.setDayCreated(details.dayCreated);

				player.setBoughtLargePlot(details.boughtLargePlot);
				player.setBoughtMediumPlot(details.boughtMediumPlot);
				player.setBoughtSmallPlot(details.boughtSmallPlot);
				...
				if (details.quests != null) {
					for (Quests attribute : details.quests.keySet()) {
						player.getQuestManager().questStages.put(attribute, details.quests.get(attribute));
					}
				}
				
				if (details.pointsReset == false) {
					player.setWeaponPoints(0);
					player.setMoneyPouch((long) 0);
					player.setPestPoints(0);
					player.setBountyPoints(0);
					player.setskillPoints(0);
					player.setpvmPoints(0);
					player.setbossPoints(0);
					player.setthievePoints(0);
					player.setfarmingPoints(0);
					player.setwoodcuttingPoints(0);
					player.setfiremakingPoints(0);
					player.setfishingPoints(0);
					player.setSmithingPoints(0);
					player.setminingPoints(0);
					player.setfletchingPoints(0);
					player.setcraftingPoints(0);
					player.setherblorePoints(0);
					player.setpuroPoints(0);
					player.setrunecraftingPoints(0);
					player.setprayerPoints(0);
					player.setSlayerPoints(0);
					player.setCredits(0);
					player.setMoneySpent(0);
					player.setSufferingCharges(0);
					player.setBlackMarks(0);
					player.getMagic().setDragonFireShieldCharges(0);
					player.setVotePoints(0);
					player.setDeaths(0);
					player.setKills(0);
					player.setRogueKills(0);
					player.setRogueRecord(0);
					player.setHunterKills(0);
					player.setHunterRecord(0);
					player.pointsReset = true;	
					save(player);
				}			
		...
		private final boolean ipSet;
		private final boolean nameSet;
		private final boolean emailSet;
		private final boolean recoverySet;
		...
		private final HashMap<Quests, Integer> quests;

		private final HashMap<Object, Object> playerProperties;
		private final boolean pointsReset;
		...
		private final Location recallLocation;
		private final boolean boughtLargePlot;
		private final boolean boughtMediumPlot;
		private final boolean boughtSmallPlot;
		private final int sufferingCharges;
		...
		public PlayerDetails(Player player) {
			...
			nameSet = player.nameSet;
			ipSet = player.ipSet;
			recoverySet = player.recoverySet;
			emailSet = player.emailSet;
			pins = player.getPin();
			...
			boughtLargePlot = player.hasBoughtLargePlot();
			boughtMediumPlot = player.hasBoughtMediumPlot();
			boughtSmallPlot = player.hasBoughtSmallPlot();
			...
			recallLocation = player.getRecallLocation();
			...
			puroPoints = player.getpuroPoints();
			...
			sufferingCharges = player.getSufferingCharges();
			...
			for (Quests attribute : player.getQuestManager().questStages.keySet()) {
				playerProperties.put(attribute, player.getQuestManager().questStages.get(attribute));			
			}
