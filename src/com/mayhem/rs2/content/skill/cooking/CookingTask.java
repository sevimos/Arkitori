...
public class CookingTask extends Task {
	...
	private void cookFood() {
		...
		AchievementHandler.activate(player, AchievementList.COOK_250_FOODS, 1);
		AchievementHandler.activate(player, AchievementList.COOK_1000_FOODS, 1);
	}
	...
	public static void handleMonkeyChild(Player player) {
		int random = Utility.randomNumber(4000);
		if (random == 1) {
			if (player.getInventory().hasSpaceFor(new Item(4033))) {
				player.getInventory().add(new Item(4033));
				player.getClient().queueOutgoingPacket(new SendMessage("@blu@You receive a pet monkey!"));
			} else if (player.getBank().hasSpaceFor((new Item(4033)))) {
				player.getBank().add((new Item(4033)));
				player.getClient().queueOutgoingPacket(new SendMessage("@blu@You receive a pet monkey! It has been sent to your bank."));
			}
			World.sendGlobalMessage("@mbl@" + player.determineIcon(player) + " " + player.getUsername() + " has recieved a pet monkey while cooking!");
		}
	}
}
