package com.mayhem.rs2.content.skill.fletching;
...
import com.mayhem.rs2.content.skill.fletching.fletchable.Fletchable;
import com.mayhem.rs2.content.skill.fletching.fletchable.FletchableItem;
...
public enum Fletching {
	...
	public static void handlePetDrop(Player player) {
		...
		if (random == 1) {
			if (player.getInventory().hasSpaceFor(new Item(4179))) {
				player.getInventory().add(new Item(4179));
				player.getClient().queueOutgoingPacket(new SendMessage("@blu@You receive a pet Broom?!"));
			} else if (player.getBank().hasSpaceFor((new Item(4179)))) {
				player.getBank().add((new Item(4179)));
				player.getClient().queueOutgoingPacket(new SendMessage("@blu@You receive pet Broom! It has been sent to your bank."));
			}
			World.sendGlobalMessage("@mbl@" + player.determineIcon(player) + " " + player.getUsername() + " has recieved a the skill pet Broom while Fletching!");
		}
			}
	
	/*public static void handleBloodMoney(Player player) {
		if (player.getInventory().hasSpaceFor(new Item(13307))) {
			player.getInventory().add(new Item(13307, Utility.randomNumber(20)));
		} else if (player.getBank().hasSpaceFor((new Item(13307)))) {
			player.getBank().add((new Item(13307, Utility.randomNumber(20))));
