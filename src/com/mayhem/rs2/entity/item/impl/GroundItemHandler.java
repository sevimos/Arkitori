package com.mayhem.rs2.entity.item.impl;
...
public class GroundItemHandler {
	...
	public static boolean add(Item item, Location location, Player player) {
		...
		if (player.getEquipment().contains(12785) && groundItem.getItem().getId() == 13307 && player.getInventory().hasSpaceFor(groundItem.getItem())) {
			player.send(new SendMessage("<col=f3ba0c>You ring places " + item.getAmount() + " blood money in your backpack."));
	...
	/**
	 * name = player username your searching for specific = if this item is
	...
	public static boolean visible(Player player, GroundItem groundItem) {
		...
		if ((player.withinRegion(groundItem.getLocation())) && (player.getLocation().getZ() == groundItem.getLocation().getZ()) && ((groundItem.isGlobal()) || ((owner != null) && (player.equals(owner))))) {
			return true;
		}
